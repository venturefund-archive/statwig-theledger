      ...
      checkToken(req, res, async (result) => {
        if (result.success) {
          ...
          permission_request = {
            result: result,
            permissionRequired:
              requiredPermissions.getInventoryDetailsForProduct,
          };
...
exports.addProductsToInventory = [
  ...
  async (req, res) => {
    try {
      ...
      permission_request = {
        role: req.user.role,
        permissionRequired: requiredPermissions.addProductsToInventory,
      };
      checkPermissions(permission_request, async (permissionResult) => {
        if (permissionResult.success) {
    ...
    try {
      checkToken(req, res, async (result) => {
        if (result.success) {
          permission_request = {
            result: result,
            permissionRequired: requiredPermissions.addInventoriesFromExcel,
          };
          checkPermissions(permission_request, async (permissionResult) => {
            if (permissionResult.success) {
              const dir = `uploads`;
    ...
    try {
      checkToken(req, res, async (result) => {
        if (result.success) {
          logger.log(
            "info",
            "<<<<< InventoryService < InventoryController < trackProduct : token verified successfullly, querying data by key"
          );

          permission_request = {
            result: result,
            permissionRequired: requiredPermissions.trackProduct,
          };
          checkPermissions(permission_request, async (permissionResult) => {
            if (permissionResult.success) {
          ...
          logger.log(
            "warn",
            "<<<<< InventoryService < InventoryController < trackProduct : refuted token"
          );
          res.status(403).json(result);
        }
      });
    ...
    try {
      checkToken(req, res, async (result) => {
        if (result.success) {
          logger.log(
            "info",
            "<<<<< InventoryService < InventoryController < getInventoryDetails : token verified successfullly, querying data by key"
          );

          permission_request = {
            result: result,
            permissionRequired: requiredPermissions.getInventoryDetails,
          };
          checkPermissions(permission_request, async (permissionResult) => {
            if (permissionResult.success) {
          ...
          logger.log(
            "warn",
            "<<<<< InventoryService < InventoryController < getInventoryDetails : refuted token"
          );
          res.status(403).json(result);
        }
      });
...
exports.getCountryDetailsByRegion = [
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        "error",
        "<<<<< InventoryService < InventoryController < getCountryDetailsByRegion : error (catch block)"
      );
      return apiResponse.ErrorResponse(res, err);
    }
  },
...
exports.getWarehouseDetailsByRegion = [
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        "error",
        "<<<<< InventoryService < InventoryController < getWarehouseDetailsByRegion : error (catch block)"
      );
      return apiResponse.ErrorResponse(res, err);
    }
  },
...
exports.getWarehouseDetailsByCountry = [
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        "error",
        "<<<<< InventoryService < InventoryController < getWarehouseDetailsByCountry : error (catch block)"
      );
      return apiResponse.ErrorResponse(res, err);
    }
  },
