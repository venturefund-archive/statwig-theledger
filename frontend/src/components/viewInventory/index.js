...
import { formatDate, getDateStringForMongo } from "../../utils/dateHelper";
						...
						<>
							<div key={i} className="col-12 p-2 mb-3 rounded row bg-white shadow">
								...
								<div className="col-2 txt1">
									{inv.inventoryDetails.quantity ? inv.inventoryDetails.quantity : 0}
									{" ("}
									{inv.products.unitofMeasure ? inv.products.unitofMeasure.name : "N/A"}
									{")"}
								</div>
								<div className="col-0 txt">
									{more[i] ? (
										<div className="col-0 txt">
											<button
												type="button"
												onClick={() => toggleShowMore(inv.id, inv.products.id, i, more[i])}
