  ...
  async (req, res) => {
    checkToken(req, res, async (result) => {
      if (result.success) {
        logger.log(
          "info",
          "<<<<< AnalyticsService < AnalyticsController < fetchAllShippingOrders  : token verified successfullly, querying data by key"
        );

        permission_request = {
          result: result,
          permissionRequired: requiredPermissions.getAnalytics,
        };
        ...
        logger.log(
          "warn",
          "<<<<< AnalyticsService < AnalyticsController < fetchAllShippingOrders  : refuted token"
        );
  ...
  async (req, res) => {
    checkToken(req, res, async (result) => {
      if (result.success) {
        logger.log(
          "info",
          "<<<<< AnalyticsService < AnalyticsController < getOverviewAnalytics  : token verified successfullly, querying data by key"
        );

        permission_request = {
          result: result,
          permissionRequired: requiredPermissions.getOverviewAnalytics,
        };
        checkPermissions(permission_request, async (permissionResult) => {
          if (permissionResult.success) {
            try {
              ...
              logger.log(
                "error",
                "<<<<< AnalyticsService < AnalyticsController < getOverviewAnalytics : error (catch block)"
              );
              console.log(err);
              return apiResponse.ErrorResponse(res, err);
            }
        ...
        logger.log(
          "warn",
          "<<<<< AnalyticsService < AnalyticsController < getOverviewAnalytics  : refuted token"
        );
        res.status(403).json(result);
      }
    });
  ...
  async (req, res) => {
    checkToken(req, res, async (result) => {
      if (result.success) {
        logger.log(
          "info",
          "<<<<< AnalyticsService < AnalyticsController < getInventoryAnalytics  : token verified successfullly, querying data by key"
        );

        permission_request = {
          result: result,
          permissionRequired: requiredPermissions.getInventoryAnalytics,
        };
        checkPermissions(permission_request, async (permissionResult) => {
          if (permissionResult.success) {
            try {
              ...
              if (batchExpiringThisWeek.length !== 0) {
                inventory.batchExpiringThisWeek =
                  batchExpiringThisWeek[0].total;
              }
              ...
              if (batchExpiringThisMonth.length !== 0) {
                inventory.batchExpiringThisMonth =
                  batchExpiringThisMonth[0].total;
              }
              ...
              if (batchExpiringInThreeMonths.length !== 0) {
                inventory.batchExpiringInThreeMonths =
                  batchExpiringInThreeMonths[0].total;
              }
              ...
              if (batchExpiringInSixMonths.length !== 0) {
                inventory.batchExpiringInSixMonths =
                  batchExpiringInSixMonths[0].total;
              }
              ...
              if (batchExpiredLastMonth.length !== 0) {
                inventory.batchExpiredLastMonth =
                  batchExpiredLastMonth[0].total;
              }
              ...
              logger.log(
                "error",
                "<<<<< AnalyticsService < AnalyticsController < getInventoryAnalytics : error (catch block)"
              );
              console.log(err);
              return apiResponse.ErrorResponse(res, err);
            }
        ...
        logger.log(
          "warn",
          "<<<<< AnalyticsService < AnalyticsController < getInventoryAnalytics  : refuted token"
        );
        res.status(403).json(result);
      }
    });
  ...
  async (req, res) => {
    checkToken(req, res, async (result) => {
      if (result.success) {
        logger.log(
          "info",
          "<<<<< AnalyticsService < AnalyticsController < getShipmentAnalytics  : token verified successfullly, querying data by key"
        );

        permission_request = {
          result: result,
          permissionRequired: requiredPermissions.getShipmentAnalytics,
        };
        checkPermissions(permission_request, async (permissionResult) => {
          if (permissionResult.success) {
            try {
              ...
              logger.log(
                "error",
                "<<<<< AnalyticsService < AnalyticsController < getShipmentAnalytics : error (catch block)"
              );
              console.log(err);
              return apiResponse.ErrorResponse(res, err);
            }
        ...
        logger.log(
          "warn",
          "<<<<< AnalyticsService < AnalyticsController < getShipmentAnalytics  : refuted token"
        );
        res.status(403).json(result);
      }
    });
