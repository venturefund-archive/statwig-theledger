...
const SoChainOfCustody = (props) => {
  ...
  return (
    <>
      {op >= level ?
        <div className={`row orderTxt  mr-0 `} style={{marginLeft: (parentIndex == 0 ? 0.5 : 1.5)+"rem", paddingLeft:  "0.5rem"}}>
          <span className={`pt-${index > 2 ? 2 : index} p-0 ${pindex == 1 && `border-primary border-left`}`} style={{ height: parentIndex == 0 && '1rem' }}></span>
