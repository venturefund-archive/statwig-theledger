...
export default function Filter({ title, filters, filterKey, setStockType, setStockId }) {
	...
	const CategoryWithoutDuplicate = filters?.reduce((finalArray, current) => {
		let obj = finalArray.find((item) => item.productCategory === current.productCategory);
	...
	const manufatcturesWithoutDuplicate = filters?.reduce((finalArray, current) => {
		let obj = finalArray.find((item) => item.manufacturerId === current.manufacturerId);
				...
				{filterKey === "productCategory" ? (
					<>
						{CategoryWithoutDuplicate?.map((item) => {
							return (
								<Dropdown.Item
									value={filterKey === "productName" ? item.productId : item[`${filterKey}`]}
									eventKey={filterKey === "productName" ? item.productId : item[`${filterKey}`]}
								>
									{item[`${filterKey}`]}
								</Dropdown.Item>
							);
					...
					<>
						{manufatcturesWithoutDuplicate?.map((item) => {
							return (
								<Dropdown.Item
									value={filterKey === "manufacturer" ? item.manufacturerId : item[`${filterKey}`]}
									eventKey={
										filterKey === "manufacturer" ? item.manufacturerId : item[`${filterKey}`]
									}
								>
									{item[`${filterKey}`]}
								</Dropdown.Item>
					...
					<>
						{filters?.map((item) => {
							return (
								<Dropdown.Item
									value={filterKey === "productName" ? item.productName : item[`${filterKey}`]}
									eventKey={filterKey === "productName" ? item.productName : item[`${filterKey}`]}
								>
									{item[`${filterKey}`]}
								</Dropdown.Item>
							);
