...
const CreateShipment = (props) => {
	...
	const [receiverOrganisation, setReceiverOrganisation] = useState();
	...
	const [FromOrgLabel, setFromOrgLabel] = useState("Select Organization Location");

	const [enableFromNewOrg, setEnablFromeNewOrg] = useState(false);

	const [openFromOrgModal, setFromOrgModal] = useState(false);
	const [openToOrgModal, setToOrgModal] = useState(false);
  ...
  const onWarehouseChange = async (value) => {
	...
	const newSenderOrganisation = (data) => {
		const body = {
			org: { name: data.name },
			warehouse: {
		...
		axios
			...
			.then((res) => {
				setSenderOrganisation({ value: res.data.data[0].id, label: res.data.data[0].name });
				setCreateNewWareNOrgSuccess(res.data.data[0].id);
        setFromOrgModal(false);
        onSenderOrgChange(res.data.data[0].id);
			})
			.catch((err) => console.log(err));
	};

	const newReceiverOrganization = (data) => {
		const body = {
			org: { name: data.name },
			warehouse: {
				country: data.country,
		...
		axios
			...
			.then((res) => {
				setReceiverOrganisation({ value: res.data.data[0].id, label: res.data.data[0].name });
				setCreateNewWareNOrgSuccess(res.data.data[0].id);
        setToOrgModal(false);
        onOrgChange(res.data.data[0].id)
			})
			.catch((err) => console.log(err));
  };
	...
	return (
		<div className="NewShipment">
												...
												{intelEnabled && (
													<Select
														...
														onChange={(v) => {
															...
															setFieldValue("fromOrgLoc", "");
															setSenderOrganisation({ value: v.value, label: v.label });
														...
														placeholder={
															disabled
																? senderOrganisation?.value?.split("/")[1]
																: t("select") + " " + t("organisation_name")
														}
														defaultInputValue={senderOrganisation?.value}
														value={
															!senderOrganisation || senderOrganisation?.value === ""
																? t("select") + " " + t("organisation_name")
																: {
																		value: senderOrganisation?.value,
																		label: senderOrganisation?.label,
																  }
														}
														options={allOrganisations.filter((a) => a.name.length > 1)}
													/>
												)}
											</div>
										</div>
                      ...
                      >
                        ...
                        /> */}
												<Select
													...
													placeholder={
														disabled
															? receiverOrganisation?.value.split("/")[1]
															: t("select") + " " + t("organisation_name")
													}
													//placeholder={"Select Organisation Name"}
													value={
														!receiverOrganisation || receiverOrganisation?.value === ""
															? t("select") + " " + t("organisation_name")
															: {
																	value: receiverOrganisation?.value,
																	label: receiverOrganisation?.label,
															  }
													}
													onChange={(v) => {
														// if (v.label === 'New Org') {
														...
														setFieldValue("toOrgLoc", "");
														setReceiverOrganisation({ value: v.value, label: v.label });
														// setReceiverOrgId(v.label);
														setFieldValue("toOrg", v.value);
														onOrgChange(v.value);
													}}
													defaultInputValue={receiverOrganisation?.value}
													options={allOrganisations.filter((a) => a.name.length > 0)}
												/>
									...
									<div className="col-md-6 com-sm-12 mt-2">
										...
										<input
											className={`input refship ${
												errors.airWayBillNo && touched.airWayBillNo ? "border-danger" : ""
											}`}
											type="text"
											id="referenceShipmentId"
											name="airWayBillNo"
                      ...
                      *{t("no_products_available")}
...

							{
								<>
									<EditTable
										check="0"
										warehouseID={senderOrganisation?.label}
										product={addProducts}
										t={t}
			...
			{fromAddLocation && (
				...
				>
					<AddLocationCard
						{...props}
						popup={true}
						senderOrgId={senderOrganisation.value}
						close={() => setFromAddLocation(false)}
					/>
				</Modal>
			)}
			{openFromOrgModal && (
				<Modal
					isMandatory={true}
					close={() => setFromOrgModal(false)}
					size="modal-md" //for other size's use `modal-lg, modal-md, modal-sm`
				>
					<OrganisationPopUp
						onHide={() => setFromOrgModal(false)}
						enableFromNewOrg={openFromOrgModal}
						onSignup={newSenderOrganisation}
					/>
				</Modal>
			)}
			{openToOrgModal && (
				<Modal
					isMandatory={true}
					close={() => setToOrgModal(false)}
					size="modal-md" //for other size's use `modal-lg, modal-md, modal-sm`
				>
					<OrganisationPopUp
						onHide={() => setToOrgModal(false)}
						enableFromNewOrg={openToOrgModal}
						onSignup={newReceiverOrganization}
					/>
				</Modal>
			)}
		</div>
