...
const EditRow = (props) => {
	...
	if (typeof products != "undefined" && typeof productsList != "undefined") {
		...
		let uniqueObjArray = [...new Map(new_products.map((item) => [item["label"], item])).values()];
                ...
                /> : prod.name
								...
								{enableDelete ? (
									<Select
										...
										className="no-border"
										placeholder={
											<div className="select-placeholder-text"> {t("product_name")}</div>
										}
										value={
					...
					<div className="">
						{showModal && (
							<div>
								<Modal
									close={closeModal}
									title={(t("fetch") === "Fetch" ? "Fetch" : "obtener") + " " + t("batch_details")}
												...
												<div className="rTable pt-1">
													<div>
														...
														>
															...
															<div className="col txt" style={{ position: "relative", left: "0%" }}>
																{ModelProd?.name}
															</div>
															<div
																className="col txt1"
