...
async function calculateRating(value, schema) {
  if (value && schema) {
    if (schema) {
      ...
      } else if (schema.man.operator == "<" && value < schema.max.value) {
        return parseInt(schema.max.rating);
...
async function calculateDirtyBottlesAndBreakage(supplierOrg) {
  try {
    ...
    if (shipmentUpdates.length) {
      for (const shipmentUpdate of shipmentUpdates) {
    ...
    if (damagedShipments.length) {
      for (const shipmentUpdate of damagedShipments) {
        for (let product of shipmentUpdate.products) {
...
exports.updateTargetSales = [
  ...
  async function (req, res) {
    try {
      for (const uniqueTarget of req.body) {
        await AnalyticsModel.updateMany(
